model{
  ...
  for(i in Nstart:Nend){
    ...
    # for(j in 1:Nlag){ # covariates days into the past
  ...
  # Rearrange covariate weights into weights at the daily, weekly, monthly, and yearly scales.
  ...
  # 
  # for(j in 1:6){
  #   wV.monthly[j] <- equals(j,1)*sum(wV[1:4]) + (1-equals(j,1))*wV[j+3] # a weight that sums over the first 4 weeks, then gets the monthly weights from before that 
  # }
  # 
  # for(j in 1:6){
  #   wV.yearly[j] <- equals(j,1)*sum(wV[1:4]) + (1-equals(j,1))*wV[j+3] 
  # }
