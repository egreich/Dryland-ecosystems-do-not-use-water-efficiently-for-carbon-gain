model{
  ...
  for(i in Nstart:Nsplitstart){
    ...
    # Main effect parts:
    for(j in 1:Nparms){
      X.effect[j,i]<- beta1[j,Tperiod[i]]*X[j,i]
    }
    ...
    dYdVPD[i] <- beta1[1,Tperiod[i]] + 2*beta1a[1]*VPD[Yday[i]] + beta2[1,1]*Tair[Yday[i]] + beta2[2,1]*PPTant[i] + beta2[3,1]*Sshall[Yday[i]] + beta2[4,1]*Sdeep[Yday[i]]
    dYdT[i]   <- beta1[2,Tperiod[i]] + 2*beta1a[2]*Tair[Yday[i]] + beta2[1,1]*VPD[Yday[i]] + beta2[5,1]*PPTant[i] + beta2[6,1]*Sshall[Yday[i]] + beta2[7,1]*Sdeep[Yday[i]]
    dYdP[i]   <- beta1[3,Tperiod[i]] + beta2[2,1]*VPD[Yday[i]] + beta2[5,1]*Tair[Yday[i]] + beta2[8,1]*Sshall[Yday[i]] + beta2[9,1]*Sdeep[Yday[i]]
    dYdSs[i]  <- beta1[4,Tperiod[i]] + beta2[3,1]*VPD[Yday[i]] + beta2[6,1]*Tair[Yday[i]] + beta2[8,1]*PPTant[i] + beta2[10,1]*Sdeep[Yday[i]]
    dYdSd[i]  <- beta1[5,Tperiod[i]] + beta2[4,1]*VPD[Yday[i]] + beta2[7,1]*Tair[Yday[i]] + beta2[9,1]*PPTant[i] + beta2[10,1]*Sshall[Yday[i]]
  ...
  # sum across days
  Dsum <- sum(sqdiff[c(Nstart:Nsplitstart,Nsplitend:N)])
  
  # Compute quantities for calculating Bayesian R2
  var.pred <- pow(sd(ET.pred[c(Nstart:Nsplitstart,Nsplitend:N)]),2)
  var.resid <- 1/tau.ET
